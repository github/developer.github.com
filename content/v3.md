...
``` command-line
$ curl -i https://{{ site.data.variables.product.api_url_pre }}/users/octocat/orgs
...
``` command-line
$ curl -i "https://{{ site.data.variables.product.api_url_pre }}/repos/vmg/redcarpet/issues?state=closed"
```
...
``` command-line
$ curl -i -u username -d '{"scopes":["public_repo"]}' https://{{ site.data.variables.product.api_url_pre }}/authorizations
```
...
``` command-line
$ curl https://{{ site.data.variables.product.api_url_pre }}
```
...
``` command-line
$ curl -u "username" https://{{ site.data.variables.product.api_url_pre }}
```

### OAuth2 Token (sent in a header)

``` command-line
$ curl -H "Authorization: token OAUTH-TOKEN" https://{{ site.data.variables.product.api_url_pre }}
```

### OAuth2 Token (sent as a parameter)

``` command-line
$ curl https://{{ site.data.variables.product.api_url_pre }}/?access_token=OAUTH-TOKEN
```
...
``` command-line
$ curl 'https://{{ site.data.variables.product.api_url_pre }}/users/whatever?client_id=xxxx&client_secret=yyyy'
```
...
``` command-line
$ curl -i https://{{ site.data.variables.product.api_url_pre }} -u foo:bar
...
``` command-line
$ curl -i https://{{ site.data.variables.product.api_url_pre }} -u valid_username:valid_password
...
``` command-line
$ curl 'https://{{ site.data.variables.product.api_url_pre }}/user/repos?page=2&per_page=100'
```
...
``` command-line
$ curl -i https://{{ site.data.variables.product.api_url_pre }}/users/whatever
> HTTP/1.1 200 OK
...
``` command-line
$ curl -i 'https://{{ site.data.variables.product.api_url_pre }}/users/whatever?client_id=xxxx&client_secret=yyyy'
> HTTP/1.1 200 OK
> Date: Mon, 01 Jul 2013 17:27:06 GMT
> Status: 200 OK
> X-RateLimit-Limit: 5000
...
``` command-line
$ curl -iH 'User-Agent: ' https://{{ site.data.variables.product.api_url_pre }}/meta
...
``` command-line
$ curl -i https://{{ site.data.variables.product.api_url_pre }}/user
> HTTP/1.1 200 OK
...
$ curl -i https://{{ site.data.variables.product.api_url_pre }}/user -H 'If-None-Match: "644b5b0155e6404a9cc4bd9d8b1ae730"'
...
$ curl -i https://{{ site.data.variables.product.api_url_pre }}/user -H "If-Modified-Since: Thu, 05 Jul 2012 15:31:30 GMT"
> HTTP/1.1 304 Not Modified
> Cache-Control: private, max-age=60
> Last-Modified: Thu, 05 Jul 2012 15:31:30 GMT
> Status: 304 Not Modified
...
``` command-line
$ curl -i https://{{ site.data.variables.product.api_url_pre }} -H "Origin: http://example.com"
HTTP/1.1 302 Found
...
``` command-line
$ curl -i https://{{ site.data.variables.product.api_url_pre }} -H "Origin: http://example.com" -X OPTIONS
...
``` command-line
$ curl -H "Time-Zone: Europe/Amsterdam" -X POST https://{{ site.data.variables.product.api_url_pre }}/repos/github/linguist/contents/new_file.md
```

...
The Gist API limits the amount of content returned for files that exceed one megabyte. Every call to retrieve a gist through the API has a key called `truncated`. If `truncated` is `true`, the file is too large and only a portion of the contents were returned in `content`.

If you need the full contents of the file, you can make a `GET` request to the URL specified by `raw_url`. Note that this only works for files less than ten megabytes. For larger files, you'll need to clone the gist via the URL provided by `git_pull_url`.
